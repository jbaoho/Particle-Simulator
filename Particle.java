...
public class Particle {
	 ...
	 * or infinity if the two particles will never collide given their current velocities.
	 * @param sideWall true if looking for a collision with the right or left walls, false otherwise
	 * @param width the width and height constraint of the particle simulation
	 * @return the time with the particle will collide with a wall, or infinity if they will never collide
	 */
	public double getWallCollisionTime(boolean sideWall, int width) {
		double t;
		if (sideWall) {
			// moving towards right wall
			if (this._vx > 0) {
				t = (width - this._x - this._radius) / this._vx;
			}
			// moving towards left wall
			else if (this._vx < 0){
				t = (this._x - this._radius) / -this._vx;
			} else {
				return Double.POSITIVE_INFINITY;
			}
		} else {
			// moving towards bottom wall
			if (this._vy > 0) {
				t = (width - this._y - this._radius) / this._vy;
			}
			// moving towards top wall
			else if (this._vy < 0){
				t = (this._y - this._radius) / -this._vy;
			} else {
				return Double.POSITIVE_INFINITY;
			}
		}
	...
	/**
	 * Updates the particle's appropriate velocity after a collision between it and the wall.
	 * @param sideWall true if looking for a collision with the right or left walls, false otherwise
	 */
	public void updateAfterWallCollision(double now, boolean sideWall) {
		if (sideWall) {
			this._vx = -_vx;
		} else {
			this._vy = -_vy;
